...
class OscStreamer {
    ...
    // ─── OSC single-float message builder ─────────────────────────────────────

    /**
     * Build a minimal OSC 1.0 message carrying a single float value.
     * Used by IMP-11 (face blend shapes) and IMP-5 (velocity channel).
     *
     * Layout: [oscString address] [",f\0\0"] [float32 BE]
     */
    private fun buildOscFloat(address: String, value: Float): ByteArray {
        val addrB = SchemaAddresses.oscString(address)
        val buf   = ByteBuffer.allocate(addrB.size + TYPE_F.size + 4).order(ByteOrder.BIG_ENDIAN)
        buf.put(addrB)
        buf.put(TYPE_F)
        buf.putFloat(value)
        return buf.array()
    ...
    fun sendRppg(amplitude: Float, bpm: Int) {
        if (!isStreaming) return
        val skt = socket ?: return; val addr = resolvedAddress ?: return
        val msg = buildOscMixedV27("/rppg", floatArrayOf(amplitude), intArrayOf(bpm))
        runCatching { skt.send(DatagramPacket(msg, msg.size, addr, port)) }
    }
    ...
    fun sendPlanes(planes: List<Pair<FloatArray, String>>) {
        ...
        planes.take(3).forEachIndexed { i, (coeffs, label) ->
            val msg = buildOscFloatStringMsg("/plane/$i", coeffs, label)
            runCatching { skt.send(DatagramPacket(msg, msg.size, addr, port)) }
        }
    }

    private fun padOscBytes(bytes: ByteArray): ByteArray {
    ...
    private fun buildOscFloatArray(address: String, values: FloatArray): ByteArray {
        val addrPad = padOscBytes(address.toByteArray(Charsets.US_ASCII) + 0)
        val typePad = padOscBytes((",".toByteArray() + "f".repeat(values.size).toByteArray()) + 0)
    ...
    private fun buildOscMixedV27(address: String, floatArgs: FloatArray, intArgs: IntArray): ByteArray {
        val addrPad = padOscBytes(address.toByteArray(Charsets.US_ASCII) + 0)
        val typeStr = "," + "f".repeat(floatArgs.size) + "i".repeat(intArgs.size)
        val typePad = padOscBytes(typeStr.toByteArray() + 0)
    ...
    private fun buildOscFloatStringMsg(address: String, floats: FloatArray, str: String): ByteArray {
        val addrPad = padOscBytes(address.toByteArray(Charsets.US_ASCII) + 0)
        ...
        val strPad = padOscBytes(str.toByteArray(Charsets.US_ASCII) + 0)

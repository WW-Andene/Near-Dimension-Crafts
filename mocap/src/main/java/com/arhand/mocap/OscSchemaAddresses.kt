...
internal object SchemaAddresses {
    ...
    // ── v27 OSC addresses ─────────────────────────────────────────────────────
    //
    // New addresses ported from fused-sl-v27 protocol extension.

    /** `/camera/pose` — 6 floats: tx, ty, tz, rx, ry, rz (SLAM accumulated pose). */
    val cameraPoseAddr:      ByteArray = oscString("/camera/pose")
    ...
    /** `/face/N/pose_body_relative` — 3 floats: yaw, pitch, roll relative to body. Indexed [0,1]. */
    val facePoseBodyRelAddr: Array<ByteArray> = Array(2) { i -> oscString("/face/$i/pose_body_relative") }

    /** `/hand/N/occlusion` — 21 floats (0=clear, 1=occluded). Indexed [0,1]. */
    val handOcclusionAddr:   Array<ByteArray> = Array(2) { i -> oscString("/hand/$i/occlusion") }

    /** `/plane/N` — 4 floats (nx,ny,nz,d) + string label. Indexed [0,1,2]. */
    val planeAddr:           Array<ByteArray> = Array(3) { i -> oscString("/plane/$i") }
